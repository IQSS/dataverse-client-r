...
Instead, to read the same file but its original version, specify `original = TRUE` and set a `FUN` argument. In this case, we know that `nlsw88.tab` is a Stata `.dta` dataset, so we will use the `haven::read_dta` function.
...
Of course, when the dataset is not ingested (such as a Rds file), users would always need to specify a `FUN` argument for the specific file.
...
In some cases, you may not want to read in the data in your environment, perhaps because that is not possible (e.g. for a `.docx` file), and you want to simply write these files your local disk. To do this, use the more primitive `get_file_*` commands. The arguments are equivalent, except we no longer need a `FUN` argument
