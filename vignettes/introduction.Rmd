...
They can be accessed from [CRAN](http://cran.r-project.org/web/packages/dataverse/index.html) or from within R using `vignettes(package = "dataverse")`.

The dataverse client package can be installed from [CRAN](http://cran.r-project.org/web/packages/dataverse/index.html), and you can find the latest development version and report any issues on GitHub:
...
(Note: **dataverse** is the next-generation iteration of [the **dvn** package](http://cran.r-project.org/package=dvn), which works with Dataverse 3 ("Dataverse Network") applications. See the appendix for a cross-walk of functionality between **dvn** and **dataverse**.)
...
If we want to restrict this to material authored by Harvard political scientist Gary King with a title that contains "ecological inference" we can more narrowly request:

```{r}
dataverse_search(author = "Gary King", title = "Ecological Inference")
```

And if we only want to search datasets, we can specify a `type`:

```{r}
dataverse_search(author = "Gary King", title = "Ecological Inference", type = "dataset")
```

To achieve retrieve a data file, we need to investigate the dataset being returned and look at what files it contains:
