...
This vignette shows how to download data from Dataverse using the dataverse package. For the purposes of this vignette, we'll focus on the Dataverse that contains supplemental files for Jamie Monogan's book *Political Analysis Using R*, which is stored at Harvard University's [IQSS Dataverse Network](https://dataverse.harvard.edu/):
...
Note: All of these UNFs indicate that the file- and study-level UNF signatures were generated using UNFv6.
...
This shows that there are indeed 32 files, a mix of .R code files and tab- and comma-separated data files. We'll focus here on the code file for Chapter 2 from the book. Let's start by grabbing the code using `get_file()` (note that this always returns a raw vector):
